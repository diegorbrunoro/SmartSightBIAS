...
def fornecedores_dados_download_farm(request):
    print("Iniciando execução da função fornecedores_dados_download_farm.")
    ...
    while True:
        ...
        if iteracao >= max_paginas:
            ...
            requests.post(consolidate_url, json={
                "module": "fornecedor",
                "output_file": "fornecedor/consolidado/fornecedores_inicial.parquet"
            })
        ...
        if (iteracao - iteracao_inicial) >= max_iteracoes_por_execucao:
            ...
            reinvoke_url = "https://southamerica-east1-quick-woodland-453702-g2.cloudfunctions.net/fornecedores-dados-download-farm"
        ...
        try:
            ...
            try:
                ...
                print(f"Erro ao decodificar JSON da resposta: {str(e)}, Resposta: {response.text}")
                raise
        ...
        if num_registros_retornados != qtd_registros:
            print(f"Aviso: Retornados {num_registros_retornados} registros na iteração {iteracao_str}, esperado {qtd_registros}.")
