...
def estoque_dados_download_farm(request) -> Tuple[str, int]:
    """
    Função que orquestra o download de dados de estoque para múltiplas filiais.
    ...
    if request_json:
        modulo = request_json.get('modulo', 'estoque')
        primeiro_registro = request_json.get('primeiroRegistro', 0)
    else:
        modulo = request.args.get('modulo', 'estoque')
        try:
