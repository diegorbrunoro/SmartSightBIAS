...
def compras_dados_download_farm(request) -> Tuple[str, int]:
    """
    Função que orquestra o download de dados de compras para múltiplas filiais.
    ...
    if request_json:
        modulo = request_json.get('modulo', 'compra')
        primeiro_registro = request_json.get('primeiroRegistro', 0)
    else:
        modulo = request.args.get('modulo', 'compra')
        try:
